            ...
            <ul className='flex flex-col mt-28 gap-5 relative w-full px-5'>
              {
                user?.user?.auth?.role === 'ADMIN_ROLE' ?
                  routesAdmin.map((r, i) => {
                    return (
                      <div key={i} className='relative'>
                        <Link onClick={() => { toggleNav() }} to={r.path} className='font-[500] text-[1rem] rounded-md transition-all duration-200 li-license p-2 cursor-pointer flex gap-1 items-center max-h-[40px]'>
                          {r.svg}
                          {r.name}
                        </Link>
                      </div>
